...
function InAppWalletConnectUI(props) {
    ...
    const otpUserInfo = state?.emailLogin
        ...
        : state?.phoneLogin
            ...
            : undefined;
    if (otpUserInfo) {
        return (_jsx(InAppWalletOTPLoginUI, { userInfo: otpUserInfo, locale: localeQuery.data, done: props.done, goBack: goBackToMain, wallet: props.wallet }));
    }
    if (state?.passkeyLogin) {
        return (_jsx(InAppWalletPassKeyLogin, { wallet: props.wallet, done: props.done, onBack: goBackToMain }));
    }
    if (state?.socialLogin) {
        return (_jsx(InAppWalletSocialLogin, { socialAuth: state.socialLogin.type, locale: localeQuery.data, done: props.done, goBack: goBackToMain, wallet: props.wallet, state: state }));
    }
    return (_jsx(InAppWalletFormUIScreen, { select: () => { }, locale: localeQuery.data, done: props.done, goBack: props.goBack, wallet: props.wallet }));
}
