...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    // A Bool value to center placeholder position in textfiled
    //MARK:Added this for center placeholder position
    @IBInspectable dynamic open var isCenterPlaceholder: Bool = false {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    */
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        var titleHeight = self.titleHeight()
        
        if isCenterPlaceholder { //MARK:Added this condition for center placeholder position
            if !(self.text?.isEmpty ?? true) {
                titleHeight = self.titleHeight()
            }else {
                titleHeight = self.frame.height * 0.06
            }
        }
    ...
    */
    open func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if editing {
            return CGRect(x: LeftPadding, y: self.frame.height * 0.05, width: bounds.size.width, height: titleHeight())
        }
...
//        return 0.0
        if let titleLabel = titleLabel,
            ...
            return font.lineHeight + self.frame.height * 0.05
        }
        return 15.0
    }
